...
class MCSubgraph {
 private:
  static constexpr size_t adj_list_buf_capacity = 1024;
  struct AdjListBuf {
    std::array<Edge, adj_list_buf_capacity> edges;
    size_t size = 0;
  };
  ...
  int num_threads;  // Number of CUDA Streams
  ...
  std::vector<AdjListBuf> adj_list_bufs;
 ...
 public:
  ...
  void insert_adj_edges(int thr_id, node_id_t src, std::vector<node_id_t> dst_vertices);
  ...
  const std::set<Edge>& get_adjlist();
  // const std::unordered_set<node_id_t>& get_neighbor_nodes(node_id_t src) { return adjlist[src]; }
