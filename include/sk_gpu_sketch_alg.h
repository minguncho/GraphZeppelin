...
public:
  SKGPUSketchAlg(node_id_t _num_nodes, size_t _num_updates, int num_threads, Bucket* buckets, size_t seed, SketchParams sketchParams, CCAlgConfiguration config = CCAlgConfiguration()) : CCSketchAlg(_num_nodes, seed, buckets, config){ 
    ...
    cudaUpdateParams = new CudaUpdateParams(num_nodes, num_updates, buckets, num_samples, num_buckets, num_columns, bkt_per_col, num_threads, 0, batch_size, 1, 1);
    ...
    gpuErrchk(cudaMallocHost(&h_edgeUpdates, 2 * num_updates * sizeof(vec_t)));
    gpuErrchk(cudaMalloc(&d_edgeUpdates, 2 * num_updates * sizeof(vec_t)));

    // Initialize buffer with 0
    memset(h_edgeUpdates, 0, 2 * num_updates * sizeof(vec_t));
