...
TaggedUpdateBatch EdgeStore::insert_adj_edges(node_id_t src,
  ...
  if (ret.size() == 0 && true_min_subgraph < cur_subgraph && needs_contraction < num_vertices) {
    ...
    check_if_too_big();
    return {src, cur_first_es_subgraph - 1, cur_first_es_subgraph - 1, ret};
  }
}
...
TaggedUpdateBatch EdgeStore::vertex_advance_subgraph(node_id_t cur_first_es_subgraph) {
  ...
  while (true) {
    ...
    if (src >= num_vertices) {
      ...
      return {0, cur_first_es_subgraph - 1, cur_first_es_subgraph - 1,
              std::vector<SubgraphTaggedUpdate>()};
    }
  ...
  std::lock_guard<std::mutex> lk(adj_mutex[src]);
  return {src, cur_first_es_subgraph - 1, cur_first_es_subgraph - 1, vertex_contract(src)};
}
