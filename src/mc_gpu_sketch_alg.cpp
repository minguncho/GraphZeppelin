...
void MCGPUSketchAlg::complete_update_batch(int thr_id, const TaggedUpdateBatch &updates) {
  ...
  node_id_t max_depth = 0;
  for (auto dst_data : dsts_data) {
    max_depth = std::max(dst_data.depth, max_depth);
    vec_t edge_id = static_cast<vec_t>(concat_pairing_fn(src_vertex, dst_data.dst));
    for (size_t graph_id = 0; graph_id <= std::min(dst_data.depth, cur_subgraphs.load()); graph_id++) {
...
void MCGPUSketchAlg::apply_update_batch(int thr_id, node_id_t src_vertex,
  ...
  if (cur_subgraphs == 0) {
    edge_store.insert_adj_edges(src_vertex, dst_vertices);
    return;
  }
  ...
  for (vec_t i = 0; i < dst_vertices.size(); i++) {
    ...
    size_t depth = Bucket_Boruvka::get_index_depth(edge_id, 0, num_subgraphs-1);
    max_depth = std::max(depth, max_depth);

    if (depth > cur_subgraphs) {
      // Adj. list
      store_edges.push_back({depth, dst_vertices[i]});
    } else {
      sketch_edges.push_back({depth, dst_vertices[i]});
    }
  }
