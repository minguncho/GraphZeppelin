...
int main(int argc, char **argv) {
  if (argc != 5 && argc != 6) {
    ...
    std::cout << "Arguments: stream_file, graph_workers, reader_threads, num_repeat, [num_batch_per_buffer]" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  int num_repeat = std::atoi(argv[4]);
  ...
  std::thread querier(track_insertions, num_updates * num_repeat, &driver, ins_start);

  for (int repeat_it = 0; repeat_it < num_repeat; repeat_it++) {
    stream.seek(0); // Reset binary stream
  ...
  std::cout << "Updates per second:           " << (stream.edges() * num_repeat) / num_seconds << std::endl;
  ...
  out << (stream.edges() * num_repeat) / num_seconds / 1e6 << ", " << memory << ", " << cc_time.count() 
      << std::endl;
}
